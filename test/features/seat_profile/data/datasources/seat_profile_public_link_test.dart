...
import 'package:sparrowkaizen/features/seat_profile/data/repositories/seat_profile_repository_impl.dart';
import 'package:sparrowkaizen/features/seat_profile/domain/usecases/get_seat_profiles_usecase.dart';

void main() {
  late _Executor executor;
  late GetSeatProfilesUseCase useCase;
  setUp(() {
    executor = _Executor();
    useCase = GetSeatProfilesUseCase(
      SeatProfileRepositoryImpl(
        SeatProfileRemoteDataSource(apiCallExecutor: executor),
      ),
    );
  });
  ...
  test(
    ...
    () async {
      expect(
        await useCase.getSeatProfilePublicLink('job-id'),
        '${ApiEndPoints.publicWebBaseUrl}/shared/seat-profile/public-id',
      );
      expect(executor.type, ApiCallType.get);
      expect(executor.fresh, isTrue);
      expect(executor.endpoint, 'job/job-id/public-links/seat-profile/');
      expect(executor.authToken, isNull);
      expect(
        await useCase.createSeatProfilePublicLink('job-id'),
        '${ApiEndPoints.publicWebBaseUrl}/shared/seat-profile/public-id',
      );
  ...
  test('DELETE accepts a successful empty response', () async {
    ...
    await useCase.deleteSeatProfilePublicLink('job-id');
  ...
  test('inactive and absent links produce the creation state', () async {
    ...
    expect(await useCase.getSeatProfilePublicLink('job-id'), isNull);
    executor.error = ApiError.requestFailed(404);
    expect(await useCase.getSeatProfilePublicLink('job-id'), isNull);
    executor.error = ApiError.requestFailed(403);
    await expectLater(
      useCase.getSeatProfilePublicLink('job-id'),
  ...
  test(
    ...
    () async {
      for (final response in [
        ...
        {'active': true, 'view_type': 'seat_profile', 'public_path': ''},
      ]) {
        executor.response = response;
        await expectLater(
          useCase.createSeatProfilePublicLink('job-id'),
          throwsA(isA<ApiError>()),
        );
      }
    },
...
class _Executor extends ApiCallExecutor {
  Object? response = {
    ...
    'public_path': '/shared/seat-profile/public-id',
  };
  ApiError? error;
  ApiCallType? type;
  String? endpoint;
