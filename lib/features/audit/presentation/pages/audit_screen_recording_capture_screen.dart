...
class AuditScreenRecordingCaptureScreen extends StatefulWidget {
  const AuditScreenRecordingCaptureScreen({super.key});

  @override
  State<AuditScreenRecordingCaptureScreen> createState() =>
      _AuditScreenRecordingCaptureScreenState();
}

class _AuditScreenRecordingCaptureScreenState extends State<AuditScreenRecordingCaptureScreen> {
  File? _recordedMedia;
            ...
            child: Column(
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      AppBackButton(onPressed: _isProcessing ? null : _handleBack),
                      const Expanded(
                    ...
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: hasRecordedMedia
                            ...
                            : _RecordingPrompt(isBusy: _isProcessing, isRecording: _isRecording),
                      ),
                    ),
                  ),
                ),
                Padding(
                  ...
                  child: _buildBottomActions(canSave: canSave, hasRecordedMedia: hasRecordedMedia),
                ),
              ],
            ),
          ),
  ...
  Widget _buildBottomActions({required bool canSave, required bool hasRecordedMedia}) {
    if (_isProcessing) {
      return SizedBox(
        height: 44,
        child: Center(
    ...
    if (_isRecording) {
      return _RecordingActionButton(
        label: '${AppStrings.auditStopRecording} ${_formatRecordingDuration(_recordingDuration)}',
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            if (isBusy)
              SizedBox(width: 30, height: 30, child: FastCircularProgressIndicator())
            else ...[
              AppTextView.body1(
                isRecording
              ...
              AppTextView.body2(
                isRecording ? AppStrings.auditStopPrompt : AppStrings.auditRecordingPrompt,
...
class _RecordedMediaFallback extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      color: Colors.black,
      child: const Center(
        child: Icon(Icons.play_circle_fill_rounded, color: Colors.white70, size: 60),
      ),
    );
  }
}
