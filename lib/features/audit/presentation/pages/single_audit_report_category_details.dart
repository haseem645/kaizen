...
class SingleAuditReportCategoryDetailsScreen extends StatefulWidget {
  const SingleAuditReportCategoryDetailsScreen({
    super.key,
  ...
  @override
  State<SingleAuditReportCategoryDetailsScreen> createState() =>
      _SingleAuditReportCategoryDetailsScreenState();
}

class _SingleAuditReportCategoryDetailsScreenState
    extends State<SingleAuditReportCategoryDetailsScreen> {
  ...
  @override
  void didUpdateWidget(covariant SingleAuditReportCategoryDetailsScreen oldWidget) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.textPrimary, size: 24),
          ],
        ),
      ),
    );
  ...
  Widget _buildCategoriesSection() {
    if (_isLoadingCategories) {
      return SizedBox(height: 132, child: Center(child: FastCircularProgressIndicator()));
    }
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return _AuditCategorySelectorCard(
                item: item,
        ...
        Expanded(
          child: FutureBuilder<List<SingleAuditReportCategoryDetails>>(
            ...
            builder: (context, snapshot) {
              ...
              final items = snapshot.data ?? const <SingleAuditReportCategoryDetails>[];
              if (items.isEmpty) {
                return _CategoryDetailsFeedback(
  ...
  Future<void> _openQuarterYearPicker() async {
    ...
    final quarter = int.tryParse(parts.last.trim().toUpperCase().replaceFirst('Q', ''));
  ...
  List<String> _yearQuarterOptions() {
    ...
    return years
        .expand(
          (year) => List<String>.generate(4, (index) => '$year - Q${index + 1}', growable: false),
        )
  ...
  Future<void> _loadCategories() async {
    ...
    try {
      final categories = await context.read<AuditController>().loadAuditReport(
      ...
      final selectedCategory =
          _resolveSelectedCategory(categories) ?? (categories.isNotEmpty ? categories.first : null);
  ...
  Future<List<SingleAuditReportCategoryDetails>> _loadDetailsForCategory(String categoryId) {
    return context.read<AuditController>().loadAuditReportCategoryDetails(
...
class _AuditCategorySelectorCard extends StatelessWidget {
  const _AuditCategorySelectorCard({
    required this.item,
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  _buildRatingBadge(value: item.stats.totalGreat, color: AppColors.green1),
                  const SizedBox(width: 10),
                  _buildRatingBadge(value: item.stats.totalAlmostThere, color: AppColors.orange1),
                  const SizedBox(width: 10),
                  _buildRatingBadge(value: item.stats.totalNeedsImprovement, color: AppColors.red1),
                  const Spacer(),
                  Container(
...
class _CategoryStatText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        AppTextView.body2(label, color: AppColors.grey1, fontSize: 12, fontWeight: FontWeight.w500),
        const SizedBox(height: 6),
        AppTextView.body1(
          value,
          color: AppColors.textPrimary,
