...
enum AuditMediaUploadFlow { descriptionComment, mediaAttachment }

class AuditMediaUploadResult {
  const AuditMediaUploadResult({
...
class AuditMediaUploadTask {
  const AuditMediaUploadTask._(this._task);
  ...
  AuditMediaUploadResult? get resultPayload =>
      _task.resultPayloadAs<AuditMediaUploadResult>();

  AuditMediaUploadFlow get flow {
    final flow = _task.metadata[_flowKey];
    return flow == 'mediaAttachment'
        ? AuditMediaUploadFlow.mediaAttachment
        : AuditMediaUploadFlow.descriptionComment;
  }

  static AuditMediaUploadTask fromBackgroundTask(
    BackgroundMediaUploadTask task,
  ) {
    return AuditMediaUploadTask._(task);
  }
}

class AuditMediaUploadController extends ChangeNotifier {
  AuditMediaUploadController._({
  ...
  static final AuditMediaUploadController instance =
      AuditMediaUploadController._();
  ...
  List<AuditMediaUploadTask> terminalTasksSince(int eventSequence) {
    return _backgroundUploadController
        ...
        .map(AuditMediaUploadTask.fromBackgroundTask)
      ...
      BackgroundMediaUploadRequest(
        ...
        finalizeUpload: (context) async {
          ...
          return BackgroundMediaUploadResult(
            payload: AuditMediaUploadResult(
              mediaUrl: mediaUrl,
              mediaType: resolvedMediaType,
            ),
          );
