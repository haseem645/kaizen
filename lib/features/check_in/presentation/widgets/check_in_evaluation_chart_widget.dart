...
class CheckInEvaluationChartWidget extends StatelessWidget {
  const CheckInEvaluationChartWidget({
    super.key,
  ...
  BarChartData _mainData(AuditEvaluationChart chart) {
    final trendsByWeek = {
      for (final trend in chart.weeklyTrends.where(
    ...
    return BarChartData(
      ...
      barGroups: List<BarChartGroupData>.generate(13, (index) {
        ...
        final needsImprovement = (trend?.needsImprovement ?? 0)
            .clamp(0, 99)
            .toDouble();
