...
public class SoundStreamPlugin : FlutterPlugin,
    ...
    private fun initRecorder() {
        ...
        enableVoiceProcessingEffects(mRecorder?.audioSessionId ?: 0)
    ...
    private fun usePhoneSpeaker(call: MethodCall, result: Result) {
        ...
        val isRecording = mRecorder?.recordingState == AudioRecord.RECORDSTATE_RECORDING
        mAudioManager?.mode = if (useSpeaker || isRecording) {
            AudioManager.MODE_IN_COMMUNICATION
        } else {
            AudioManager.MODE_NORMAL
        }
        mAudioManager?.isSpeakerphoneOn = useSpeaker
        setPlayerMuted(!useSpeaker)
        result.success(true)
    }
    ...
    private fun setPlayerMuted(muted: Boolean) {
        val volume = if (muted) 0.0f else 1.0f
        try {
            ...
            debugLog("setPlayerMuted failed: ${e.localizedMessage}")
        }
    }

    private fun enableVoiceProcessingEffects(audioSessionId: Int) {
        if (audioSessionId <= 0) {
            debugLog("Voice processing effects skipped: invalid audio session id")
            return
        }
        ...
        try {
            if (AcousticEchoCanceler.isAvailable()) {
                acousticEchoCanceler = AcousticEchoCanceler.create(audioSessionId)?.apply {
                    enabled = true
                    debugLog("AcousticEchoCanceler enabled=$enabled")
                }
            } else {
                debugLog("AcousticEchoCanceler unavailable")
            }
        } catch (e: Exception) {
            debugLog("AcousticEchoCanceler failed: ${e.localizedMessage}")
        }
        ...
        try {
            if (NoiseSuppressor.isAvailable()) {
                noiseSuppressor = NoiseSuppressor.create(audioSessionId)?.apply {
                    enabled = true
                    debugLog("NoiseSuppressor enabled=$enabled")
                }
            } else {
                debugLog("NoiseSuppressor unavailable")
            }
        } catch (e: Exception) {
            debugLog("NoiseSuppressor failed: ${e.localizedMessage}")
        }
        ...
        try {
            if (AutomaticGainControl.isAvailable()) {
                automaticGainControl = AutomaticGainControl.create(audioSessionId)?.apply {
                    enabled = true
                    debugLog("AutomaticGainControl enabled=$enabled")
                }
            } else {
                debugLog("AutomaticGainControl unavailable")
            }
        } catch (e: Exception) {
            debugLog("AutomaticGainControl failed: ${e.localizedMessage}")
        }
    }
    ...
    private fun createRecordListener(): OnRecordPositionUpdateListener {
        return object : OnRecordPositionUpdateListener {
            override fun onMarkerReached(recorder: AudioRecord) {
                audioData?.let { recorder.read(it, 0, minOf(it.size, mRecorderBufferSize)) }
            }
            ...
            override fun onPeriodicNotification(recorder: AudioRecord) {
                ...
                byteBuffer.order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().put(data, 0, shortOut)
